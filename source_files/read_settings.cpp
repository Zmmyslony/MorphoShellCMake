...
// In this way we support commenting out things in the settings file with //.
    while (true) {
        ...
        std::string::size_type first_comment_pos = file_contents.find("//");
        ...
        std::string::size_type next_newline_pos = file_contents.find('\n', first_comment_pos);
...
// or std::remove_if documentation.
    file_contents.erase(
            std::remove_if(file_contents.begin(), file_contents.end(), [](unsigned char x) { return std::isspace(x); }),
            file_contents.end());
...
// In the remaining string, find positions of all equals signs and semicolons.
    ...
    if (equalsign_positions.size() != semicolon_positions.size()) {
        throw std::runtime_error(
                "Error: After removing all comments, the settings file did not have equal numbers of = and ; \n The most common mistake is to forget a semicolon.");
    }
    ...
    if (!std::is_sorted(vec_that_should_be_ascending.begin(), vec_that_should_be_ascending.end())) {
        throw std::runtime_error(
                "Error: After removing all comments, the = and ; in the settings file did not occur in the correct order.");
    }
...
//
    ++j;
    if (setting_names_vec.at(j) != "outputs_per_char_long_time") {
        throw std::runtime_error("Error: something went wrong when reading the outputs_per_char_long_time setting.");
    }
...
//
    ++j;
    if (setting_names_vec.at(j) != "ref_thickness_if_uniform") {
        throw std::runtime_error("Error: something went wrong when reading the ref_thickness_if_uniform setting.");
    }
    stuff.ref_thickness_if_uniform = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "timestep_prefactor") {
        throw std::runtime_error("Error: something went wrong when reading the timestep_prefactor setting.");
    }
    stuff.timestep_prefactor = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "dialling_from_ansatz_rather_than_ref_state") {
        throw std::runtime_error(
                "Error: something went wrong when reading the dialling_from_ansatz_rather_than_ref_state setting.");
    }
    stuff.dialling_from_ansatz_rather_than_ref_state = boolstring_to_bool(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "dial_factors_to_hold_at_spacing") {
        throw std::runtime_error(
                "Error: something went wrong when reading the dial_factors_to_hold_at_spacing setting.");
    }
    stuff.dial_factors_to_hold_at_spacing = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "dial_phase_time_prefactor") {
        throw std::runtime_error("Error: something went wrong when reading the dial_phase_time_prefactor setting.");
    }
    stuff.dial_phase_time_prefactor = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "time_between_equil_checks_prefactor") {
        throw std::runtime_error(
                "Error: something went wrong when reading the time_between_equil_checks_prefactor setting.");
    }
    stuff.time_between_equil_checks_prefactor = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "damping_prefactor_1") {
        throw std::runtime_error("Error: something went wrong when reading the damping_prefactor_1 setting.");
    }
    stuff.damping_prefactor_1 = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "damping_prefactor_2") {
        throw std::runtime_error("Error: something went wrong when reading the damping_prefactor_2 setting.");
    }
    stuff.damping_prefactor_2 = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) !=
        "num_steps_to_use_gradient_descent_dynamics_for_before_switching_to_newtonian_dynamics") {
        throw std::runtime_error(
                "Error: something went wrong when reading the num_steps_to_use_gradient_descent_dynamics_for_before_switching_to_newtonian_dynamics setting.");
    }
    stuff.num_steps_to_use_gradient_descent_dynamics_for_before_switching_to_newtonian_dynamics = std::stoi(
            settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "force_to_char_force_ratio_equil_threshold") {
        throw std::runtime_error(
                "Error: something went wrong when reading the force_to_char_force_ratio_equil_threshold setting.");
    }
    stuff.force_to_char_force_ratio_equil_threshold = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "speed_to_char_speed_ratio_equil_threshold") {
        throw std::runtime_error(
                "Error: something went wrong when reading the speed_to_char_speed_ratio_equil_threshold setting.");
    }
    stuff.speed_to_char_speed_ratio_equil_threshold = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "def_poisson_ratio") {
        throw std::runtime_error("Error: something went wrong when reading the def_poisson_ratio setting.");
    }
    stuff.def_poisson_ratio = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "ref_shear_modulus_if_uniform") {
        throw std::runtime_error("Error: something went wrong when reading the ref_shear_modulus_if_uniform setting.");
    }
    stuff.ref_shear_modulus_if_uniform = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "ref_density") {
        throw std::runtime_error("Error: something went wrong when reading the ref_density setting.");
    }
    stuff.ref_density = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "bend_stiffening_scale_factor") {
        throw std::runtime_error("Error: something went wrong when reading the bend_stiffening_scale_factor setting.");
    }
    stuff.bend_stiffening_scale_factor = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "patch_mat_dimless_conditioning_thresh") {
        throw std::runtime_error(
                "Error: something went wrong when reading the patch_mat_dimless_conditioning_thresh setting.");
    }
    stuff.patch_mat_dimless_conditioning_thresh = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "slide_stiffness_prefactor") {
        throw std::runtime_error("Error: something went wrong when reading the slide_stiffness_prefactor setting.");
    }
    stuff.slide_stiffness_prefactor = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "slide_friction_coefficent") {
        throw std::runtime_error("Error: something went wrong when reading the slide_friction_coefficent setting.");
    }
    stuff.slide_friction_coefficent = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "slide_speed_prefactor") {
        throw std::runtime_error("Error: something went wrong when reading the slide_speed_prefactor setting.");
    }
    stuff.slide_speed_prefactor = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "using_initial_glass_slide_z_coords_from_settings_file") {
        throw std::runtime_error(
                "Error: something went wrong when reading the using_initial_glass_slide_z_coords_from_settings_file setting.");
    }
    stuff.using_initial_glass_slide_z_coords_from_settings_file = boolstring_to_bool(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "initial_upper_slide_z_coord") {
        throw std::runtime_error("Error: something went wrong when reading the initial_upper_slide_z_coord setting.");
    }
    stuff.initial_upper_slide_z_coord = std::stod(settings_vec.at(j));
//
    ++j;
    if (setting_names_vec.at(j) != "initial_lower_slide_z_coord") {
        throw std::runtime_error("Error: something went wrong when reading the initial_lower_slide_z_coord setting.");
    }
...
//
    ...
    if (stuff.dialling_from_ansatz_rather_than_ref_state) {
        if (stuff.ansatz_file_was_given) {
            ...
            throw std::runtime_error(
                    "Error: You cannot use the dialling_from_ansatz_rather_than_ref_state setting if you have not supplied an ansatz file.");
        }
    }

    if (stuff.dial_factors_to_hold_at_spacing > 1.0) {
        throw std::runtime_error(
                "Error: The largest permitted value of dial_factors_to_hold_at_spacing is 1.0, because dial_factor never goes above 1.0, so anything larger would be meaningless.");
    }
    ...
    if (stuff.num_steps_to_use_gradient_descent_dynamics_for_before_switching_to_newtonian_dynamics > 0) {
        ...
        log << "You have chosen to use gradient descent dynamics for "
            << stuff.num_steps_to_use_gradient_descent_dynamics_for_before_switching_to_newtonian_dynamics
            << " steps before switching to Newtonian dynamics." << std::endl;
    } else {
...
bool boolstring_to_bool(const std::string &boolstring) {
    if (boolstring == "true") {
        ...
        throw std::runtime_error(
                "Error: you passed a std::string to boolstring_to_bool that read neither true nor false.");
    }
}
